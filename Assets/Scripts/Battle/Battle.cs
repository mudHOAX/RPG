...
public class Battle
{
    ...
    public int CalculateAttackDamage(BaseEnemy attacker, BaseCharacter target)
    {
        Random rnd = new Random();
        int baseDamage = attacker.Stats.Attack - target.Stats.Defence;
        int bonusDamage = attacker.Stats.Strength + rnd.Next(0, (int)Math.Floor((decimal)(attacker.Level + attacker.Stats.Strength) / 4));
        return baseDamage * bonusDamage;
    }

    public int CalculateAttackDamage(BaseCharacter attacker, BaseEnemy target)
    {
        Random rnd = new Random();
        int baseDamage = 0;
    ...
    private int CalculateBonus(BaseCharacter attacker, BaseEnemy target, int bonusDamage)
    {
        ...
        // if target is weak to element of weapon   bonusDamage = Math.Floor(bonusDamage * 1.5f);
        foreach (Element element in attacker.Gear.Weapon.ElementAttack) // TODO Should this be only the weapon or all gear??
        ...
        // if target halves element of weapon       bonusDamage = Math.Floor(bonusDamage * 0.5f);
        foreach (Element element in attacker.Gear.Weapon.ElementAttack) // TODO Should this be only the weapon or all gear??
